...
from locisimiles.document import Document
from locisimiles.pipeline._types import CandidateJudgeOutput
...
def _on_query_select(
    evt: gr.SelectData, query_segments: list, matches_dict: dict
) -> tuple[dict, dict]:
        ...
        for query_row in query_segments:
            ...
            if matches:
                for match in matches:
                    ...
                    similarity = (
                        _extract_numeric_from_html(match[2])
                        if isinstance(match[2], str)
                        else match[2]
                    )
                    probability = (
                        _extract_numeric_from_html(match[3])
                        if isinstance(match[3], str)
                        else match[3]
                    )
            ...
            with gr.Column(scale=1):
                ...
                source_matches = gr.Dataframe(
                    ...
                    datatype=[
                        ...
                        "html",
                        "html",
                    ],  # Enable HTML rendering for metric columns
                )
