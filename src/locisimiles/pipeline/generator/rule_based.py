...
This generator implements a multi-stage rule-based approach:
...
class RuleBasedCandidateGenerator(CandidateGeneratorBase):
    ...
    This generator implements a multi-stage rule-based approach to detect
    potential intertextuality between Latin texts.  It combines orthographic
    ...
    def __init__(
        ...
        *,
        ...
        if self.use_htrg and not TORCH_AVAILABLE:
            raise ImportError(
                "HTRG filter requires torch and transformers. "
                "Install with: pip install torch transformers"
            )
        if self.use_similarity and not SPACY_AVAILABLE:
            raise ImportError(
                "Similarity filter requires spacy. "
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            top_k: Maximum matches per query (``None`` = no limit).
        ...
        # Convert to CandidateGeneratorOutput format
        results = self._matches_to_candidates(all_matches, source, query)
    ...
    def _matches_to_candidates(
        ...
        """Convert internal matches to ``CandidateGeneratorOutput`` format."""
        results: CandidateGeneratorOutput = {str(seg.id): [] for seg in query}
    ...
    def _normalize_quotation_marks(self, text_list: List[List[str]]) -> List[List[str]]:
        ...
        for item in text_list:
            if len(item) > 1:
                result.append([item[0], re.sub(quote_pattern, "'", item[1])])
        return result
    ...
    def _remove_whitespace_connectors(self, text_list: List[List[str]]) -> List[List[str]]:
        ...
        for item in text_list:
            if len(item) > 1:
                result.append([item[0], re.sub(r" (que|ve|ue)([ ,\.!?])", r"\1\2", item[1])])
        return result
    ...
    def _strip_whitespaces(self, text_list: List[List[str]]) -> List[List[str]]:
        ...
        for item in text_list:
            if len(item) > 1:
                result.append([item[0], item[1].strip()])
        return result
