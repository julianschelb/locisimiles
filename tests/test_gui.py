...
class TestPipelineVisibility:
    ...
    def test_all_pipeline_choices_are_handled(self):
        """Every pipeline choice should produce a 7-tuple without errors."""
        for choice in PIPELINE_CHOICES:
            result = _update_pipeline_visibility(choice)
            assert len(result) == 7

    def test_two_stage_shows_both_models(self):
        desc, emb, cls, retr, rb, w2v, ctx = _update_pipeline_visibility(PIPELINE_TWO_STAGE)
    ...
    def test_exhaustive_shows_classification_only(self):
        desc, emb, cls, retr, rb, w2v, ctx = _update_pipeline_visibility(PIPELINE_EXHAUSTIVE)
    ...
    def test_retrieval_shows_embedding_only(self):
        desc, emb, cls, retr, rb, w2v, ctx = _update_pipeline_visibility(PIPELINE_RETRIEVAL)
        assert emb["visible"] is True
    ...
    def test_rule_based_shows_rule_params(self):
        desc, emb, cls, retr, rb, w2v, ctx = _update_pipeline_visibility(PIPELINE_RULE_BASED)
        assert emb["visible"] is False
        assert cls["visible"] is False
        assert retr["visible"] is False
    ...
    def test_word2vec_shows_word2vec_and_retrieval_params(self):
        desc, emb, cls, retr, rb, w2v, ctx = _update_pipeline_visibility(PIPELINE_WORD2VEC)
        assert emb["visible"] is False
        assert cls["visible"] is False
        assert retr["visible"] is True
        assert rb["visible"] is False
    ...
    def test_contextual_retrieval_shows_contextual_and_retrieval(self):
        desc, emb, cls, retr, rb, w2v, ctx = _update_pipeline_visibility(
            PIPELINE_LATIN_BERT_RETRIEVAL
        )
        assert emb["visible"] is False
        assert cls["visible"] is False
    ...
    def test_contextual_two_stage_shows_contextual_and_classification(self):
        desc, emb, cls, retr, rb, w2v, ctx = _update_pipeline_visibility(
            PIPELINE_LATIN_BERT_TWO_STAGE
        )
        assert emb["visible"] is False
